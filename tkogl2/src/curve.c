...
int curve_buildLine(point_t* p1, point_t* p2, point_t** line)
{
	...
	{
		/*Build the line*/
		if (*line != NULL) Tcl_Free((char*)* line); /*if memory already allocated, free it up*/
		*line = (point_t*)Tcl_Alloc(sizeof(point_t) * 2); /*allocate memory for line of size 2*/
		(*line)[0].x = p1->x;
		(*line)[0].y = p1->x;
	...
	else
	{
		...
		*line = (point_t*)Tcl_Alloc(sizeof(point_t) * size);
...
{
	curve_t* curve = (curve_t*)Tcl_Alloc(sizeof(curve_t));
	curve->points[0] = NULL;
	curve->points[1] = NULL;
	curve->points[2] = NULL;
	curve->pointNum = 0;
...
void curve_slice_amount(int amount)
{
	...
	if (amount > 0)
	{
		curves = (curve_t * *)Tcl_Alloc(amount * sizeof(curve_t*));
