...
int setWindowId(HWND hwnd)
{
	...
	/*Setting up pixel format: create pfd struct, fill out features that you want, then pass through ChoosePixelFormat to find closest match. Then assign this value using SetPixelFormat*/
	PIXELFORMATDESCRIPTOR pfd;
	memset(&pfd, 0, sizeof(pfd));
	pfd.nVersion = 1;
	pfd.dwFlags = PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENGL | PFD_DOUBLEBUFFER; /*flags*/
	pfd.iPixelType = PFD_TYPE_RGBA; /*Kind of framebuffer. RGBA or palette*/
	pfd.cColorBits = 24;
	pfd.cDepthBits = 16;
	...
	pixelNumber = pixelFormat;

	HGLRC rc = wglCreateContext(dc); /*returns a handle to the openGL context*/

	wglMakeCurrent(dc, rc); /*takes device context and rendering context to make openGL the current context*/
...
/*returns positive difference between a and b*/
...
float getRealZ(float x, float y, float z)
{
	...
	for (int i = 0; i < models[model_index].count * 3; i += 3)
	{
		if (absd(models[model_index].vertex[i], x) < 0.01
			&& absd(models[model_index].vertex[i + 1], y) < 0.01
			&& absd(models[model_index].vertex[i + 2], z) < 0.01)
		{
			return models[model_index].vertex[i + 2];
		}
	}
	return 0.0;
}
...
void drawDots()
{
	...
	dot_t* n = dot_get(-1); /*returns dot slice id*/
	int dotId = 1;
	while (n != NULL)
	{
		...
		{
			ogl_drawLabel(&n->p, c, dotId++, dotRadius, n->p.z >= 0 ? models[model_index].max[Z] : models[model_index].min[Z]);
		}
		n = n->next;
	}
}
...
/*Updates the current display with any changes to canvas (loading model, placing landmarks, etc.)*/
void onDisplay()
{
	...
	glOrtho(-1.1 * dx, 1.1 * dx, -1.1, 1.1, -2, 2); /*multiplies identity by orthographic matrix generated by given values*/
	...
	{
		/*these handle the rotations of the object by using the mouse*/
		...
		if (showModel != NONE && showModel != DOWN_SAMPLE_ONLY)
			ogl_drawModel(&models[model_index]); /*use openGL functions to draw specified model, this is the key function call that will draw the .ply file to the canvas*/
			//ogl_DrawTriangle();
		switch (showModel)
		{
		case LANDMARK:
...
int valiadteDot(point_t* p)
{
	...
	if (p->x < models[model_index].min[X] || p->x > models[model_index].max[X])
		return 0;
	if (p->y < models[model_index].min[Y] || p->y > models[model_index].max[Y])
		return 0;
	if (p->z< models[model_index].min[Z] || p->z > models[model_index].max[Z])
		return 0;
	return 1;
}

/*Tcl_SetResult: arranges for string to be the return string for the current Tcl command in interp, replacing any existing result*/
/*IF TCL_DYNAMIC it means that string was allocated with a call to Tcl_Alloc and is now the property of the Tcl system.*/
/*Tcl_SetResult will arrange for the string's storage to be released by calling Tcl_Free when it is no longer needed*/
TCL_CMD(add)
{
	const char* shape = Tcl_GetStringFromObj(objv[1], NULL);
	if (strcmp(shape, "specimen") == 0)
	{
		/*this is the part that loads the model and makes it ready to be drawn to canvas using onDisplay()*/
		int id;
		Tcl_GetIntFromObj(interp, objv[3], &id);
		ogl_loadModel(Tcl_GetStringFromObj(objv[2], NULL), &models[id]); /*Tcl_GetStringFromObj(objv[2],NULL) will give the filename, and &models[id] gives the specified model*/
		float maxXY = models[id].max[0] > models[id].max[1] ? models[id].max[0] : models[id].max[1]; /*sets max*/
		if (maxXY > 0.8)
		{
			dotRadius = 0.01f;
		}
	...
	else if (strcmp(shape, "rawdot") == 0)
	{
		...
		p.x = ogl_calCoordinate(p.x, 0, models[model_index].delta);
		p.y = ogl_calCoordinate(p.y, 1, models[model_index].delta);
		p.z = ogl_calCoordinate(p.z, 2, models[model_index].delta);
	...
	else if (strcmp(shape, "downsample") == 0)
	{
		...
		if (file)
		{
			ogl_loadDownSampleModel(file, &models[id]);
			fclose(file);
		}
	}
...
/*show seems to only be used for landmark placement*/
TCL_CMD(show)
{
...
/*objv = a named vector of Tcl objects*/

#define ANGLE_REDUCE(angle) while (angle > 180.0) angle -= 180.0; while (angle < -180.0) angle += 180.0
TCL_CMD(setSpecimen)
{
	const char* attr = Tcl_GetStringFromObj(objv[1], NULL);
	...
	/*allocates memory for specimen*/
	else if (strcmp(attr, "amount") == 0)
	{
		int amount = 0;
		...
		if (amount > 0) {
			models = (model_t*)Tcl_Alloc(amount * sizeof(model_t));
			context = (context_t*)Tcl_Alloc(amount * sizeof(context_t));
			memset(models, 0, amount * sizeof(model_t));
		}
		model_index = 0;
		model_amount = amount;
	...
	else if (strcmp(attr, "id") == 0)
	{
		...
		{
			model_index = index;
			float maxXY = models[model_index].max[0] > models[model_index].max[1] ? models[model_index].max[0] : models[model_index].max[1];
...
TCL_CMD(loadDgt)
{
	...
	if (model_amount > 0) {
		...
		int id = 0;
		float maxXY = models[id].max[0] > models[id].max[1] ? models[id].max[0] : models[id].max[1];
		if (maxXY > 0.8)
		{
			dotRadius = 0.01f;
		}
...
/*Tcl_Interp is client-visible fields of interpreter structures*/
