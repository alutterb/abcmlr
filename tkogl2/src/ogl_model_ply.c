...
/****This file handles the LOADING of .ply files. ogl_model.c handles loading and drawing****/
...
int ogl_loadDgtModel(const char* filename, model_t** models)
{
	...
	*models = (model_t*)Tcl_Alloc(amount * sizeof(model_t));
	...
	for (int i = 0; i < amount; i++)
	{
		...
		if (ogl_loadDownSampleModel(file, &(*models)[i]) <= 0)
		{
			break;
		}
	}
